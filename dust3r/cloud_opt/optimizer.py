...
from dust3r.utils.event import read_voxel_hdf5, VoxelGrid, events_to_voxel_grid, detect_harris_corners_and_gradients, detect_harris_corners_fast, generate_random_points, compute_gradient_flow_dot_product, compute_gradient_flow_dot_product_fast, batch_index_tensor, visualize_gradient_flow_dot_product, normalized_l2_loss, normalized_l2_loss_fast, visualize_flow,generate_superpoint_points
from dust3r.snr_weighted_event_loss import compute_snr_weighted_event_loss, compute_adaptive_snr_weight
...
class PointCloudOptimizer(BasePCOptimizer):
    ...
    def __init__(self, *args, optimize_pp=False, focal_break=20, shared_focal=False, flow_loss_fn='smooth_l1', flow_loss_weight=0.0, event_loss_weight=0.0,event_stream_all_path=None, use_snr_weighting=False, snr_weight_strategy="1-snr", snr_weight_type="pixel",
        ...
        self.snr_weight_strategy = snr_weight_strategy
        self.snr_weight_type = snr_weight_type
        ...
        # Caches for per-frame corner detection results to avoid recomputation across iterations
        # Keyed by (img_id, sample_type, patch_size, k, threshold, max_corners)
        self._corners_cache = {}
        self._patches_cache = {}
        self._gradients_cache = {}

        # Cache for event voxel representations keyed by (first_id, second_id)
        self._event_repr_cache = {}
        
        # Extract SNR maps from pred1 and pred2
        self.snr_maps = [None for _ in range(self.n_imgs)]
        if len(args) >= 4:  # Ensure view1, view2, pred1, pred2 parameters exist
            view1, view2, pred1, pred2 = args[0], args[1], args[2], args[3]
            if 'snr_map' in pred1 and 'snr_map' in pred2:
                for v in range(len(self.edges)):
                    idx = view1['idx'][v]
                    if v < len(pred1['snr_map']):
                        self.snr_maps[idx] = pred1['snr_map'][v]
                    idx = view2['idx'][v]
                    if v < len(pred2['snr_map']):
                        self.snr_maps[idx] = pred2['snr_map'][v]

        # Optional: warm up Harris caches for all frames to avoid the first-iter spike
        try:
            with torch.no_grad():
                for img_id in range(self.n_imgs):
                    cache_key_harris = (img_id, 'harris', 5, 0.08, 0.003, None)
                    if cache_key_harris not in self._corners_cache:
                        corners, patches, gradients = detect_harris_corners_and_gradients(
                            self.imgs[img_id], patch_size=5, k=0.08, threshold=0.003
                        )
                        self._corners_cache[cache_key_harris] = corners
                        self._patches_cache[cache_key_harris] = patches
                        self._gradients_cache[cache_key_harris] = gradients
        except Exception:
            # If warm-up fails, don't affect subsequent normal flow
            pass
    ...
    def forward_non_batchify(self, epoch=9999):
        ...
        if self.flow_loss_weight > 0 and epoch >= self.num_total_iter * self.flow_loss_start_epoch:
            ...
            # accumulators for batched event loss
            batched_ego_flows = []   # list of [2,H,W]
            batched_event_repr = []  # list of [H,W]
            batched_corners = []     # list of [Ni,2] (numpy or tensor)
            batched_gradients = []   # list of [Ni,2] (tensor or numpy)
            batched_lengths = []
            ...
            for e, (i, j) in enumerate(self.edges):
                ...
                if self.event_loss_weight > 0:
                    ...
                    # Get or cache event voxel representation, avoid repeated construction
                    cache_key_event = (first_id, second_id)
                    if cache_key_event in self._event_repr_cache:
                        event_repr = self._event_repr_cache[cache_key_event]
                    else:
                        with torch.no_grad():
                            event_repr = self.get_event(
                                self.event_indices[first_id],
                                self.event_indices[second_id],
                                self.voxel_grids[first_id]
                            )
                        self._event_repr_cache[cache_key_event] = event_repr
                                        # (intensity gradient of image) dot product flow should be similar to the event_repr_i
                    ##### TODO: Optimize this code section!! #####
                    sample_type = 'harris'  # Use manual Harris to maintain precision
                    pose_type = 'fast'      # Use fast dot product to improve speed
                    if sample_type == 'harris_fast':
                        cache_key = (first_id, 'harris_fast', 5, 0.04, 0.01, 500)
                        if cache_key in self._corners_cache:
                            corners = self._corners_cache[cache_key]
                            patches = self._patches_cache[cache_key]
                            gradients = self._gradients_cache[cache_key]
                        else:
                            with torch.no_grad():
                                corners, patches, gradients = detect_harris_corners_fast(
                                    self.imgs[first_id],
                                    patch_size=5,  # Patch size around corners
                                    k=0.04,        # Sensitivity parameter for Harris corner detection
                                    threshold=0.01, # Corner response threshold
                                    max_corners=500  # Limit maximum number of corners for speed
                                )
                            self._corners_cache[cache_key] = corners
                            self._patches_cache[cache_key] = patches
                            self._gradients_cache[cache_key] = gradients
                    elif sample_type == 'harris':
                        cache_key = (first_id, 'harris', 5, 0.08, 0.003, None)
                        if cache_key in self._corners_cache:
                            corners = self._corners_cache[cache_key]
                            patches = self._patches_cache[cache_key]
                            gradients = self._gradients_cache[cache_key]
                            ...
                            with torch.no_grad():
                                corners, patches, gradients = detect_harris_corners_and_gradients(
                                    self.imgs[first_id],
                                    patch_size=5,  # Patch size around corners
                                    k=0.08,        # Sensitivity parameter for Harris corner detection (optimized)
                                    threshold=0.003, # Corner response threshold (optimized)
                                )
                            self._corners_cache[cache_key] = corners
                            self._patches_cache[cache_key] = patches
                            self._gradients_cache[cache_key] = gradients
                    elif sample_type == 'random':
                        # Use random points for comparison experiments
                        corners, patches, gradients = generate_random_points(
                            self.imgs[first_id],
                            patch_size=5,
                            num_points=50,
                        )
                    elif sample_type == 'superpoint':
                        corners, patches, gradients = generate_superpoint_points(
                            self.imgs[first_id],
                            patch_size=5,
                            device='cuda'  # Specify GPU usage
                        )
                    
                    # Defer event loss calculation, collect batch data first
                    if pose_type == 'previous':
                        # Compatible with old version: still collect here, use fast vectorized implementation in batch processing
                        pass
                    elif pose_type == 'fast':
                        pass

                    # Only collect when corners exist
                    if corners is not None and len(corners) > 0:
                        batched_ego_flows.append(ego_flow)            # [2,H,W]
                        batched_event_repr.append(event_repr[0].to(ego_flow.device))       # [H,W]
                        batched_corners.append(corners)
                        batched_gradients.append(gradients)
                        batched_lengths.append(len(corners))
                    # # Visualize RAFT flow
                    # visualize_flow(
                    #     raft_flow,
                    #     self.imgs[first_id],
                    ...
                    # # Visualize gradient and flow dot product
            ...
            # Batch event loss calculation (vectorized indexing, reduce kernel launches and Python loops)
            if self.event_loss_weight > 0 and len(batched_lengths) > 0:
                device = batched_ego_flows[0].device
                # stack flows and event repr
                flow_stack = torch.stack(batched_ego_flows, dim=0)           # [E,2,H,W]
                event_stack = torch.stack(batched_event_repr, dim=0).to(device)         # [E,H,W]

                # concat corners/gradients
                offsets = []
                b_idx = []
                x_all_list = []
                y_all_list = []
                grad_all_list = []
                for bi, (corn, grad) in enumerate(zip(batched_corners, batched_gradients)):
                    if not isinstance(corn, torch.Tensor):
                        corn = torch.from_numpy(corn)
                    if not isinstance(grad, torch.Tensor):
                        grad = torch.from_numpy(grad)
                    corn = corn.to(device)
                    grad = grad.to(device)
                    x_all_list.append(corn[:, 0].long())
                    y_all_list.append(corn[:, 1].long())
                    grad_all_list.append(grad)
                    b_idx.append(torch.full((corn.shape[0],), bi, device=device, dtype=torch.long))
                x_all = torch.cat(x_all_list, dim=0).to(device)
                y_all = torch.cat(y_all_list, dim=0).to(device)
                b_idx = torch.cat(b_idx, dim=0)
                grads_all = torch.cat(grad_all_list, dim=0).to(device)                  # [Ntot,2]

                # batched indexing
                flow_x = flow_stack[b_idx, 0, y_all, x_all]
                flow_y = flow_stack[b_idx, 1, y_all, x_all]
                dot_all = grads_all[:, 0] * flow_x + grads_all[:, 1] * flow_y  # [Ntot]

                evt_all = event_stack[b_idx, y_all, x_all]                    # [Ntot]

                # split per pair and accumulate loss
                splits = torch.split(dot_all, batched_lengths)
                evt_splits = torch.split(evt_all, batched_lengths)
                event_loss_total = torch.zeros((), device=device)
                
                # Try using SNR-weighted event_loss
                use_snr_weighting = self.use_snr_weighting and any(snr_map is not None for snr_map in self.snr_maps)
                
                if use_snr_weighting:
                    try:
                        # Get corner coordinates
                        corner_coords = torch.stack([x_all, y_all], dim=1)  # [Ntot, 2]
                        
                        # Process SNR weighting by image pair
                        start_idx = 0
                        edge_idx = 0
                        for dp, ev, length in zip(splits, evt_splits, batched_lengths):
                            if ev.numel() > 0 and dp.numel() > 0:
                                # Get corresponding corner coordinates
                                end_idx = start_idx + length
                                pair_corners = corner_coords[start_idx:end_idx]
                                
                                # Get current image pair index
                                if edge_idx < len(self.edges):
                                    i, j = self.edges[edge_idx]
                                    
                                    # Try to get SNR map from first image
                                    snr_map = self.snr_maps[i]
                                    if snr_map is None:
                                        # If first image has no SNR map, try second image
                                        snr_map = self.snr_maps[j]
                                    
                                    if snr_map is not None:
                                        if snr_map.dim() > 2:
                                            snr_map = snr_map.squeeze(0)  # Remove batch dimension
                                        
                                        # Use SNR-weighted loss calculation
                                        # Select strategy based on weighting method
                                        if hasattr(self, 'snr_weight_strategy'):
                                            snr_strategy = self.snr_weight_strategy
                                        else:
                                            snr_strategy = "1-snr"  # Default: higher weight for low SNR
                                        if hasattr(self, 'snr_weight_type'):
                                            snr_type = self.snr_weight_type
                                        else:
                                            snr_type = "pixel"
                                        snr_weighted_loss = compute_snr_weighted_event_loss(
                                            ev, dp, snr_map, pair_corners, 
                                            snr_weight_strategy=snr_strategy,
                                            snr_weight_type=snr_type
                                        )
                                        event_loss_total = event_loss_total + snr_weighted_loss
                                    else:
                                        # If no SNR map, use standard loss
                                        event_loss_total = event_loss_total + normalized_l2_loss_fast(ev, dp)
                                else:
                                    # If cannot get image pair index, use standard loss
                                    event_loss_total = event_loss_total + normalized_l2_loss_fast(ev, dp)
                        ...
                        print(f"Warning: SNR weighting failed, falling back to standard loss: {e}")
                        # Fall back to standard loss calculation
                        for dp, ev in zip(splits, evt_splits):
                            event_loss_total = event_loss_total + normalized_l2_loss_fast(ev, dp)
                else:
                ...
                # Average over number of edges
                event_loss = event_loss_total / self.n_edges
            else:
