...
class M3ED(BaseStereoViewDataset):
    def __init__(self,
        ...
        # 'split1': {'indoor_flying1': (139, 1201),   # 1 (140, 1200) VALIDATION, TEST
        #            'indoor_flying2': (159, 1581),  # 1 (160, 1580) TRAIN
        #            'indoor_flying3': (124, 1816),   # 1 (125, 1815) TRAIN
        #            'indoor_flying4': (90, 360)},   # NOT USED
        print('loading m3ed dataset...')
        super().__init__(*args, **kwargs)
        self.dataset_label = 'm3ed'
        self.split = dset
        ...
        environments = {
            ...
            # "outdoor_night":["high_beams",
            #                 "penno_parking_1",
            #                 "penno_parking_2"]
        }
        # vehicle = "spot"
        # environments = {
        #     "outdoor_day":["penno_short_loop",
        #                     "skatepark_1","skatepark_2",
        #                     "srt_green_loop",
        #                     "srt_under_bridge_1","srt_under_bridge_2",
        #                     "art_plaza_loop","rocky_steps"],
        # }
        self.sequences = []
        for environment in environments.keys():
            for seq in environments[environment]:
                self.sequences.append(os.path.join(dataset_location,f"{vehicle}_{environment}_{seq}")) 
        ...
        for seq in self.sequences:
            ...
            if self.split == 'train':
                # First 60% for training set
                split_index = int(len(os.listdir(rgb_path)) * 0.6)
            else:
                # Last 40% for test set
                split_index = int(len(os.listdir(rgb_path)) * 0.4)
            for stride in strides:
                for ii in range(0,split_index-1-self.S*stride+1, clip_step):
        ...
        # NOTE: only one seq in each sequences
        intrinsics = [1033.3937072541314, 1031.9143859001108, 556.0270133221886, 309.24746130253124]
    ...
    def _get_views(self, index, resolution, rng):
        ...
        for i in range(2):
            ...
            # value_channel = hsv_image[:, :, 2]  # Use Value channel
