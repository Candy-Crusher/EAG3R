...
def get_args_parser():
    ...
    parser.add_argument('--use_lowlight_enhancer', action='store_true', default=False, help='evaluate wE for pose evaluation')
    parser.add_argument('--event_enhance_mode', default='none', type=str, help='evaluate wE for pose evaluation')
    ...
    parser.add_argument('--use_dynamic_weight', action='store_true', default=False, help='use dynamic event loss weight based on SNR and corner count')
    parser.add_argument('--use_snr_weighting', action='store_true', default=False, help='use SNR map to weight event loss pixels directly')
    parser.add_argument('--snr_weight_strategy', default='1-snr', type=str, choices=['snr', '1-snr'], help='SNR weighting strategy: snr (high SNR weight more) or 1-snr (low SNR weight more)')
    parser.add_argument('--snr_weight_type', default='pixel', type=str, choices=['pixel', 'region'], help='SNR weighting type: pixel (weight each pixel) or region (weight each region)')
    ...
    parser.add_argument('--eval_dataset', type=str, default='sintel', 
                    choices=['carla3d','mvsec','davis', 'kitti', 'bonn', 'scannet', 'tum', 'nyu', 'sintel'], 
    ...
    parser.add_argument('--seqidx', default=0, type=int, help="sequence index")
    return parser
