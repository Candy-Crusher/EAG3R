...
from dust3r.dynamic_weight import calculate_dynamic_event_loss_weight, get_snr_and_corners_from_model, log_dynamic_weight_info
...
def eval_pose_estimation_dist(args, model, device, all_img_path, seq_img_path, save_dir=None, mask_path=None):
    ...
    for seq in tqdm(seq_list):
        try:
            dir_path = metadata['dir_path_func'](seq_img_path, seq, args.seqidx)
            ...
            if args.use_event_control:
                event_path = metadata['event_path_func'](seq_img_path, seq, args.seqidx)
            ...
            dynamic_event_loss_weight = args.event_loss_weight
            if args.event_loss_weight > 0 and args.use_dynamic_weight:
                try:
                    # Get SNR and corner information
                    snr_mean, corner_count = get_snr_and_corners_from_model(model, imgs)
                    
                    # Calculate dynamic weight
                    dynamic_event_loss_weight = calculate_dynamic_event_loss_weight(
                        snr_mean, corner_count, args.event_loss_weight
                    )
                    
                    # Log dynamic weight adjustment information
                    seq_id = getattr(args, 'seqidx', 0)
                    log_dynamic_weight_info(seq_id, snr_mean, corner_count, 
                                          args.event_loss_weight, dynamic_event_loss_weight)
                except Exception as e:
                    print(f"Warning: Failed to calculate dynamic weight: {e}")
                    print(f"Using original weight: {args.event_loss_weight}")
                    dynamic_event_loss_weight = args.event_loss_weight
            ...
            with torch.enable_grad():
                if len(imgs) > 2:
                    ...
                    scene = global_aligner(
                        ...
                        event_loss_weight=dynamic_event_loss_weight,event_stream_all_path=event_stream_all_path,
                        use_snr_weighting=getattr(args, 'use_snr_weighting', False),
                        snr_weight_strategy=getattr(args, 'snr_weight_strategy', '1-snr'),
                        snr_weight_type=getattr(args, 'snr_weight_type', 'pixel'),
            ...
            gt_traj_file = metadata['gt_traj_func'](seq_img_path, anno_path, seq, args.seqidx)
