...
class BasicLayer(nn.Module):
    ...
    """
    def __init__(self, dim, depth, num_heads, window_size=7, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0.,
...
class SWINPad(nn.Module):
    ...
    def __init__(self, pretrain_img_size=224, patch_size=4, in_chans=3, embed_dim=96, depths=[2, 2, 6, 2],
                 ...
                 attn_drop_rate=0., drop_path_rate=0.2, norm_layer=partial(nn.LayerNorm, eps=1e-6), ape=False, patch_norm=True,
                 out_indices=(0, 1, 2, 3), frozen_stages=-1, use_checkpoint=False, pretrained_checkpoint=None, pretrained_checkpoint_type=None, keep_patch_keys=True, load_teacher=False, num_classes=0, *args,**kwargs):
        super().__init__()
        print(args,kwargs)
        ...
        if pretrained_checkpoint is not None:
            ...
            if pretrained_checkpoint_type == 'event':
                print("Loading event pretrained checkpoint")
            ...
            if pretrained_checkpoint_type == 'img_segpretrained':
                print("Loading image segmentation pretrained checkpoint")
