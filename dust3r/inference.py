...
# Add fvcore import for FLOPs calculation
try:
...
def loss_of_one_batch(batch, model, criterion, device, symmetrize_batch=False, use_amp=False, ret=None, 
                      measure_flops=False, measure_latency=False, measure_vram=False):
    view1, view2 = batch
    # this condition is established when all data from the batch is from the same dataset split

    ignore_keys = set(['depthmap', 'dataset', 'label', 'instance', 'idx', 'true_shape', 'rng', 'event_index', 'img_ts'])
    for view in batch:
    ...
    # Measure VRAM usage (before inference)
    if measure_vram and torch.cuda.is_available():
        torch.cuda.synchronize()
        vram_before = torch.cuda.memory_allocated() / (1024**3)  # Convert to GB
        vram_reserved_before = torch.cuda.memory_reserved() / (1024**3)  # Convert to GB
    ...
    with torch.cuda.amp.autocast(enabled=bool(use_amp)):
        # Measure FLOPs
        if measure_flops and FVCORE_AVAILABLE:
            flops = FlopCountAnalysis(model, (view1, view2))
    ...
    if measure_vram and torch.cuda.is_available():
        ...
        vram_reserved_after = torch.cuda.memory_reserved() / (1024**3)  # Convert to GB
        vram_peak = torch.cuda.max_memory_allocated() / (1024**3)  # Peak memory usage
        
        print(f"VRAM Usage:")
        print(f"  Allocated: {vram_after:.3f} GB (peak: {vram_peak:.3f} GB)")
        print(f"  Reserved: {vram_reserved_after:.3f} GB")
        print(f"  Delta (allocated): {vram_after - vram_before:.3f} GB")
        print(f"  Delta (reserved): {vram_reserved_after - vram_reserved_before:.3f} GB")
...
@torch.no_grad()
def inference(pairs, model, device, batch_size=8, verbose=True, measure_flops=False, measure_latency=False, measure_vram=False):
    if verbose:
    ...
    for i in tqdm.trange(0, len(pairs), batch_size, disable=not verbose):
        ...
        res = loss_of_one_batch(collate_with_cat(pairs[i:i+batch_size]), model, None, device,
                               measure_flops=measure_flops, measure_latency=measure_latency, measure_vram=measure_vram)
